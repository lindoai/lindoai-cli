...
export function createWorkflowsCommand(): Command {
  ...
  // workflows status-website <record-id>
  workflows
    .command('status-website <record-id>')
    .description('Check the status of a website-creation workflow (started via create-website)')
    .option('-f, --format <format>', 'Output format (json, table)', 'table')
    .action(async (recordId: string, options: { format: OutputFormat }) => {
      const client = getClient();
      try {
        const status = await client.workflows.getWebsiteStatus(recordId);
        if (!status.done) info(`Status: ${status.status} — ${status.message}`);
        else success(status.message);
        output(status, options.format);
      } catch (err) {
        handleError(err);
      }
  ...
  // workflows status-page <record-id>
  workflows
    .command('status-page <record-id>')
    .description('Check the status of a page-creation workflow (started via create-page)')
    .option('-f, --format <format>', 'Output format (json, table)', 'table')
    .action(async (recordId: string, options: { format: OutputFormat }) => {
      const client = getClient();
      try {
        const status = await client.workflows.getPageStatus(recordId);
        if (!status.done) info(`Status: ${status.status} — ${status.message}`);
        else success(status.message);
        output(status, options.format);
      } catch (err) {
  ...
  // workflows status-blog <record-id>
  workflows
    .command('status-blog <record-id>')
    .description('Check the status of a blog-creation workflow (started via create-blog)')
    .option('-f, --format <format>', 'Output format (json, table)', 'table')
    .action(async (recordId: string, options: { format: OutputFormat }) => {
      const client = getClient();
      try {
        const status = await client.workflows.getBlogStatus(recordId);
        if (!status.done) info(`Status: ${status.status} — ${status.message}`);
        else success(status.message);
        output(status, options.format);
      } catch (err) {
  ...
  workflows
    ...
    .action(async (source: string, options: { format: OutputFormat }) => {
      ...
      try {
        ...
        success(`Batch accepted: ${res.succeeded} succeeded, ${res.failed} failed of ${res.total}.`);
  ...
  // workflows batch-status-websites <record-ids...>
  workflows
    .command('batch-status-websites <record-ids...>')
    .description('Check the status of up to 25 website workflows at once.')
    .option('-f, --format <format>', 'Output format (json, table)', 'table')
    .action(async (recordIds: string[], options: { format: OutputFormat }) => {
      const client = getClient();
      try {
        const res = await client.workflows.batchCheckWebsiteStatus(recordIds);
        if (!res.done) info(`Batch status: ${res.status} — ${res.message}`);
  ...
  // workflows batch-status-pages <record-ids...>
  workflows
    .command('batch-status-pages <record-ids...>')
    .description('Check the status of up to 25 page workflows at once.')
    .option('-f, --format <format>', 'Output format (json, table)', 'table')
    .action(async (recordIds: string[], options: { format: OutputFormat }) => {
      const client = getClient();
      try {
        const res = await client.workflows.batchCheckPageStatus(recordIds);
        if (!res.done) info(`Batch status: ${res.status} — ${res.message}`);
        else success(res.message);
        output(res, options.format);
      } catch (err) {
  ...
  // workflows batch-status-blogs <record-ids...>
  workflows
    .command('batch-status-blogs <record-ids...>')
    .description('Check the status of up to 25 blog workflows at once.')
    .option('-f, --format <format>', 'Output format (json, table)', 'table')
    .action(async (recordIds: string[], options: { format: OutputFormat }) => {
      const client = getClient();
      try {
        const res = await client.workflows.batchCheckBlogStatus(recordIds);
        if (!res.done) info(`Batch status: ${res.status} — ${res.message}`);
        else success(res.message);
        output(res, options.format);
      } catch (err) {
