...
export function createWorkflowsCommand(): Command {
  ...
  workflows
    .command('edit-website <website-id> <prompt>')
    .description('Edit a website with AI (the agent edits across its pages).')
    .option('--publish', 'Publish the changes to the live website (default: save as drafts to review + publish later)')
    .option('--schedule-at <iso>', 'Optional ISO 8601 timestamp to schedule the edit')
    .option('-f, --format <format>', 'Output format (json, table)', 'table')
    .action(async (websiteId: string, prompt: string, options: { publish?: boolean; scheduleAt?: string; format: OutputFormat }) => {
      const client = getClient();
      try {
        const req: { prompt: string; publish?: boolean; schedule_at?: string } = { prompt };
        if (options.publish) req.publish = true;
        if (options.scheduleAt) req.schedule_at = options.scheduleAt;
        info(`Starting website edit on ${websiteId}…`);
        const res = await client.workflows.editWebsite(websiteId, req);
        const id = res.result?.workflow_id;
        if (id) success(`Edit started. Poll with: workflows status-edit ${id}`);
  ...
  // workflows status-edit <workflow-id>
  workflows
    .command('status-edit <workflow-id>')
    .description('Check the status of a website-edit workflow (started via edit-website)')
    .option('-f, --format <format>', 'Output format (json, table)', 'table')
    .action(async (workflowId: string, options: { format: OutputFormat }) => {
      const client = getClient();
      try {
        const status = await client.workflows.getEditStatus(workflowId);
        const r = status.result;
        if (r && !r.done) info(`Status: ${r.status} — ${r.message}`);
        else if (r) success(r.message);
        output(status, options.format);
