...
import { extractHtmlData, preserveCanonical } from '../utils/html-extraction';
...
export function createPagesCommand(): Command {
  ...
  pages
    ...
    .option('--file <path>', 'Output file path (relative to current directory)', './page.html')
    .option('--no-background', 'Run preview server in foreground (blocks terminal)')
    .action(async (websiteId: string, pagePath: string, options: { title: string; file: string; background: boolean }) => {
      ...
      try {
        ...
        // Print website context for agent/user awareness
        printWebsiteContext(websiteResult);
  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
    ...
    <a href="mailto:hello@example.com" class="inline-flex items-center justify-center px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition text-lg">
      Contact Us
        ...
        const isDark = (websiteTheme.mode as string)?.toLowerCase() === 'dark';
        const settings = {
          theme: {
        ...
        // Determine dark/light mode from website theme (default to light)
        const themeMode = (websiteTheme.mode as string)?.toLowerCase() === 'dark' ? 'dark' : '';
...
${mainContentWithGlobalSections}
    ...
    <!-- End Page Custom Code (Footer) -->
        ...
        info(`To publish changes: lindoai pages update ${websiteId} ${pageId} --html-file ${options.file}`);
    ...
    .action(async (websiteId: string, pageId: string, options: { htmlFile?: string; uploadAssets?: boolean; format: OutputFormat }) => {
      ...
      try {
        ...
        const response = await client.pages.publish(websiteId, pageId, {
          ...
          seo: preserveCanonical(extracted.seo, pageResponse.result.seo),
...
/**
 * Prints website context summary for agent/user awareness.
 * This gives context about the business and design style
 * so an agent knows what content to generate.
 */
function printWebsiteContext(websiteResult: {
  business_name?: string;
  business_description?: string;
  language?: string;
  theme_md?: string | null;
  ...
  if (websiteResult?.business_name) {
    console.log(`Business: ${websiteResult.business_name}`);
  }
  if (websiteResult?.business_description) {
    console.log(`Description: ${websiteResult.business_description}`);
  }
  if (websiteResult?.theme_md) {
    console.log(`Design: ${websiteResult.theme_md}`);
  }
  if (websiteResult?.language) {
    console.log(`Language: ${websiteResult.language}`);
  }
  console.log('---');
  console.log('');
}
