...
import { extractGlobalSections } from './global-sections';
...
export function extractHtmlData(fullHtml: string, defaultTitle?: string): ExtractedHtmlData {
  ...
  const document = load(fullHtml);
  const canonical = document('link').filter((_, element) =>
  ...
  if (canonical !== undefined) seo.canonical_url = canonical;
...
/** Keep an existing canonical on partial updates; an explicit empty href clears it. */
export function preserveCanonical(seo: Record<string, any>, existingSeo?: Record<string, any>): Record<string, any> {
  if (seo.canonical_url === undefined && existingSeo?.canonical_url !== undefined) {
    return { ...seo, canonical_url: existingSeo.canonical_url };
  }
  return seo;
}
