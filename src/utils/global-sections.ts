 ...
 *
 * Global sections are identified either by comment markers or as direct children (root level) elements:
 * - Comment markers: `<!-- ========== GLOBAL HEADER ... -->` / `<!-- ========== END GLOBAL HEADER ... -->`
 * - Fallback: `<header>` at root level = global header, `<footer>` at root level = global footer
 *
...
export function extractGlobalSections(html: string): GlobalSections {
  let globalHeader: string | undefined;
  let globalFooter: string | undefined;
  let remaining = html;

  // Try comment-marker-based extraction first
  const headerMarkerMatch = remaining.match(
    /<!-- ========== GLOBAL HEADER[^>]*-->\s*([\s\S]*?)\s*<!-- ========== END GLOBAL HEADER[^>]*-->/i
  );
  if (headerMarkerMatch) {
    globalHeader = headerMarkerMatch[1].trim() || undefined;
    remaining = remaining.replace(headerMarkerMatch[0], '').trim();
  }

  const footerMarkerMatch = remaining.match(
    /<!-- ========== GLOBAL FOOTER[^>]*-->\s*([\s\S]*?)\s*<!-- ========== END GLOBAL FOOTER[^>]*-->/i
  );
  if (footerMarkerMatch) {
    globalFooter = footerMarkerMatch[1].trim() || undefined;
    remaining = remaining.replace(footerMarkerMatch[0], '').trim();
  }

  // Strip page content markers if present
  const pageContentMatch = remaining.match(
    /<!-- ========== PAGE CONTENT[^>]*-->\s*([\s\S]*?)\s*<!-- ========== END PAGE CONTENT[^>]*-->/i
  );
  if (pageContentMatch) {
    remaining = pageContentMatch[1].trim();
  }

  // If no comment markers found, fall back to tag-based extraction
  if (!headerMarkerMatch && !footerMarkerMatch) {
    const headerResult = extractDirectChildByTag(remaining, 'header');
    ...
    remaining = footerResult.remainingHtml.trim();
  }
 ...
 * Adds comment markers to delineate sections for extraction.
 *
...
export function injectGlobalSections(
  ...
  if (globalHeader && globalHeader.trim()) {
    parts.push(`<!-- ========== GLOBAL HEADER (shared across all pages) ========== -->`);
    parts.push(globalHeader.trim());
    parts.push(`<!-- ========== END GLOBAL HEADER ========== -->`);
    parts.push('');
  }

  // Add main content (always included, even if empty)
  parts.push(`<!-- ========== PAGE CONTENT (edit below) ========== -->`);
  if (mainContent.trim()) {
  ...
  if (globalFooter && globalFooter.trim()) {
    parts.push('');
    parts.push(`<!-- ========== GLOBAL FOOTER (shared across all pages) ========== -->`);
    parts.push(globalFooter.trim());
    parts.push(`<!-- ========== END GLOBAL FOOTER ========== -->`);
  }
